...
client.connect(('10.223.174.147',6969))  #连接服务器
        ...
        while not thread_exit:
            ...
            if image is not None:
                ...
                if len(face_encoding) > 0:
                    print("send encoding")
                    start = time.time()
                    client.send((str(face_encoding[0])).encode())
                    nm_tl = client.recv(128)
                    while len(nm_tl) < 1:
                        # wait until recieved
                        nm_tl = client.recv(128)
                    print("recieve: "+nm_tl.decode())
                    end = time.time()
                    print("time_cost: %f \n",end-start)
                    # recive name and distance
                    nm_tl = (nm_tl.decode()).split()
                    image_share.distsance = eval(nm_tl[1])
                    thread_lock.acquire()
                    if image_share.distsance< system_checker.tolerance:
                        self.face_name = nm_tl[0]
                    else:
                        self.face_name = 'Unknown'
                    thread_lock.release()
            else:
