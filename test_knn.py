...
class RecognizeThread(threading.Thread):
    def __init__(self):
        ...
        self.nei = neighbors.KNeighborsClassifier(n_neighbors=3)
    ...
    def run(self):
        ...
        self.nei.fit(known_face_encodings,known_face_names)
        image_cropper = CropImage()
        ...
        while not thread_exit:
            ...
            if image is not None:
                ...
                if len(face_encoding) > 0:
                    ...
                    # knn_match = self.nei.predict([face_encoding[0]])

                    # svm_match == known_face_names[best_match_index] and
                    if svm_match == known_face_names[best_match_index] and face_distances[best_match_index] < system_checker.tolerance:
                        thread_lock.acquire()
...
if __name__ == "__main__":
    ...
    for person_name in os.listdir(path):
        for pic_name in os.listdir(path+'/'+person_name):
            name_image = cv2.imread(path + '/' + person_name +'/' +pic_name)
            ...
            try:
                ...
                print(pic_name, 'Not Explicit Face.')
